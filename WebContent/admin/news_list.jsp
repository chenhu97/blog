...
<div class="centercontent">
	...
	<div class="pageheader">
		<h1 class="pagetitle">系统后台 - 新闻管理 - 列表</h1>
		<span class="pagedesc">列出所有新闻数据</span>

	</div>
	<!--pageheader-->

	<div id="contentwrapper" class="contentwrapper">
		<form action='news_list.jsp' method='get' id='listform'>
			<span>关键字</span>
			<input type= 'search' name='searchName' id='searchName' placeholder='请输入要搜索的关键字' />
			<input type='submit' value='搜索'/>
			<input type='button' onclick="javascript:location.href='news_list.jsp'" value='清空'/>
		
		</form>
		<button onclick="javascript:location.href='news_insert.jsp';">添加</button>
		&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<table id="dataList" cellpadding="0" cellspacing="0" border="0"
			...
			<tbody>
				<%
					int i = 1;
					for (News item : vDataList) {
				%>
				<tr>
					<td><input type="checkbox" value="<%=item.getNewsId()%>" /></td>
					<td><%=i%></td>
					<td><%=item.getNewsId()%></td>
					<td><%=item.getCatName()%></td>
					<td class="center"><%=item.getTitle()%></td>
					<td class="center"><%=item.getAuthor()%></td>
					<td class="center"><%=item.getStatus()%></td>
					<td class="center"><%=item.getUpdateOn()%></td>
					<td class="center">
					    <a href="news_delete.do?id=<%=item.getNewsId()%>">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;

						<a href="news_update.jsp?id=<%=item.getNewsId()%>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;

						<a href="news_detail.jsp?id=<%=item.getNewsId()%>">查看</a>&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
				</tr>
				<%
					i++;
