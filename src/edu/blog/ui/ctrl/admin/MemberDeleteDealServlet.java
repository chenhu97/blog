...
public class MemberDeleteDealServlet extends BaseServlet {
	 ...
	 */
	public MemberDeleteDealServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id == null || id.isEmpty()) {
			out.println("<script>alert('id不能为空。');location.href='member_list.jsp';</script>");
			return;
		}
		// 3)服务端验证 主键的合法性
		Long vId = 0L;
		try {
			...
			out.println("<script>alert('id不是数字。');location.href='member_list.jsp';</script>");
			return;
		}
		...
		if (result > 0) {
			out.println("<script>alert('删除成功。');location.href='member_list.jsp';</script>");
		} else {
			out.println("<script>alert('删除失败。');location.href='member_list.jsp'</script>");
		}
