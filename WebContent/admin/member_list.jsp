...
<%
	...
	//list<Member> dataList = memberService.list();//旧方案
...
<%
	...
	if(SysFun.isNullOrEmpty(searchName)){
		...
		//不为空，则采用搜索分页方法
		Long rowCount = memberService.countByName(searchName);
		
		pagerItem.changeRowCount(rowCount);
		
		vDataList = memberService.pagerByName(searchName, pagerItem.getPageNum(),pagerItem.getPageSize());
...
<div class="centercontent">
	...
	<div class="pageheader">
		<h1 class="pagetitle">系统后台 - 会员管理 - 列表</h1>
		<span class="pagedesc">列出所有会员数据</span>

	</div>
	<!--pageheader-->

	<div id="contentwrapper" class="contentwrapper">
		<form action='member_list.jsp' method='get' id='listform'>
			<span>关键字</span>
			<input type='search' name='searchName' id='searchName' placeholder='请输入要搜索的关键字' />
			<input type='submit' value='搜索'/>
			<input type='button' onclick="javascript:location.href='member_list.jsp'" value='清空'/>
		
		</form>
		<button onclick="javascript:location.href='member_insert.jsp';">添加</button>
		&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<table id="dataList" cellpadding="0" cellspacing="0" border="0"
			...
			<tbody>
				<%
					int i = 1;
					for (Member item : vDataList) {
				%>
				<tr>
					<td><input type="checkbox" value="<%=item.getUserId()%>" /></td>
					<td><%=i%></td>
					<td><%=item.getUserId()%></td>
					<td><%=item.getUserName()%></td>
					<td class="center"><%=item.getNickName()%></td>
					<td class="center"><%=item.getEmail()%></td>
					<td class="center"><a
						href="member_delete.do?id=<%=item.getUserId()%>">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;

						<a href="member_update.jsp?id=<%=item.getUserId()%>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;

						<a href="member_detail.jsp?id=<%=item.getUserId()%>">查看</a>&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
				</tr>
				<%
					i++;
