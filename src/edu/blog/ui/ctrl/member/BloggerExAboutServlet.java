 ...
 */
@WebServlet("/member/BloggerEx_about")
public class BloggerExAboutServlet extends HttpServlet {
	...
	private void defaultDeal(HttpServletRequest request, HttpServletResponse response)
		...
		if (vIsOk) {
			...
			out.println("location.href='BloggerEx_about';");
			out.println("</script>");
		} else {
			vMsg += "保存失败!" + vMsg;
			request.setAttribute("msg", vMsg);
	...
	private void defaultView(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("content", content);
		String toPage = "/WEB-INF/member/BloggerEx_about.jsp";
